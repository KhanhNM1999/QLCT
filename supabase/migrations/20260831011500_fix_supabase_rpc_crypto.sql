...
alter table public.qlct_app_states
  add column if not exists sync_id_hash text;
...
create unique index if not exists qlct_app_states_sync_id_hash_idx
  on public.qlct_app_states (sync_id_hash);

create index if not exists qlct_app_states_user_id_idx
...
immutable
as $$
  select encode(extensions.digest(coalesce(session_token, ''), 'sha256'), 'hex')
$$;
...
begin
  ...
  values (normalized, extensions.crypt(user_password, extensions.gen_salt('bf')))
...
begin
  ...
  if found_user_id is null or found_password_hash <> extensions.crypt(coalesce(user_password, ''), found_password_hash) then
...
create or replace function public.qlct_username_exists(user_name text)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    select 1
    from public.qlct_users
    where username = lower(regexp_replace(coalesce(user_name, ''), '[^a-z0-9._-]', '', 'g'))
  )
$$;
...
begin
  ...
  values ('user:' || current_user_id::text, current_user_id, coalesce(app_state, '{}'::jsonb), coalesce(saved_at, now()))
...
grant execute on function public.qlct_username_exists(text) to anon;
